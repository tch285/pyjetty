...
class ProcessData_ENC(process_data_base_pPb.ProcessDataBase):
  ...
  def initialize_user_output_objects(self):
    ...
    for ctype in ['P', 'M', 'PM', 'T']:
      for ptype in ptypes:
        h3_raw = ROOT.TH3D(f"raw_{ctype}_{ptype}", f"raw_{ctype}_{ptype}", self.weight_nbins, self.weight_bins, self.RL_nbins, self.RL_bins, self.pT_det_nbins, self.pT_det_bins)
        setattr(self, f"raw_{ctype}_{ptype}", h3_raw)

        h2_raw_eec = ROOT.TH2D(f"raw_eec_{ctype}_{ptype}", f"raw_eec_{ctype}_{ptype}", self.RL_nbins, self.RL_bins, self.pT_det_nbins, self.pT_det_bins)
        setattr(self, f"raw_eec_{ctype}_{ptype}", h2_raw_eec)
  ...
  def fill_jet_tables(self, jet, jet_pt_sub):
    ...
    for indices, RL, weight in zip(corr.indices(), corr.rs(), corr.weights()):
      ...
      getattr(self, f"raw_T_{ptype}").Fill(weight, RL, jet_pt_sub)
      getattr(self, f"raw_eec_T_{ptype}").Fill(RL, jet_pt_sub, weight)
      getattr(self, f"raw_{ctype}_{ptype}").Fill(weight, RL, jet_pt_sub)
      getattr(self, f"raw_eec_{ctype}_{ptype}").Fill(RL, jet_pt_sub, weight)
    ...
    if self.mixed_cone:
      ...
      for p1 in cone1_w_ptcut:
        for p2 in cone2_w_ptcut:
          ...
          ctype = self.get_charge_type(q1, q2)
          getattr(self,  "raw_T_mx").Fill(pair_weight, pair_RL, jet_pt_sub)
          getattr(self,  "raw_eec_T_mx").Fill(pair_RL, jet_pt_sub, pair_weight)
          getattr(self, f"raw_{ctype}_mx").Fill(pair_weight, pair_RL, jet_pt_sub)
          getattr(self, f"raw_eec_{ctype}_mx").Fill(pair_RL, jet_pt_sub, pair_weight)
