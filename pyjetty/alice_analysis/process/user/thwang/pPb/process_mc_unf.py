...
class ProcessMC_ENC(process_mc_base_pPb.ProcessMCBase):
  ...
  def initialize_user_output_objects_R(self, jetR):
    ...
    for ctype in ['P', 'M', 'PM', 'T']:
      for ptype in ['jj', 'jp', 'pp', 'mx']:
        # h3_reco = ROOT.TH3D(f"reco_{ctype}_{ptype}", f"reco_{ctype}_{ptype}", n_bins_reco[0], binnings_reco[0], n_bins_reco[1], binnings_reco[1], n_bins_reco[2], binnings_reco[2])
        h3_reco = ROOT.TH3D(f"reco_{ctype}_{ptype}", f"reco_{ctype}_{ptype}", self.weight_nbins, self.weight_bins, self.RL_nbins, self.RL_bins, self.pT_det_nbins, self.pT_det_bins)
        setattr(self, f"reco_{ctype}_{ptype}", h3_reco)
        # h3_gen = ROOT.TH3D(f"gen_{ctype}_{ptype}", f"gen_{ctype}_{ptype}", n_bins[0], binnings[0], n_bins[1], binnings[1], n_bins[2], binnings[2])
        h3_gen = ROOT.TH3D(f"gen_{ctype}_{ptype}", f"gen_{ctype}_{ptype}", self.weight_nbins, self.weight_bins, self.RL_nbins, self.RL_bins, self.pT_truth_nbins, self.pT_truth_bins)
        setattr(self, f"gen_{ctype}_{ptype}", h3_gen)
        # for purity correction
        name = f'reco_unmatched_{ctype}_{ptype}'
        ...
        name = f"response_{ctype}_{ptype}"
        ...
        # response = ROOT.RooUnfoldResponse(h3_reco, h3_gen, response_matrix, name, name)
  ...
  def fill_matched_jet_histograms(self, det_jet, truth_jet, rho_bge_det, rho_bge_truth):
    ...
    for d_pair in det_pairs:
       ptype = d_pair.ptype
       ctype = d_pair.ctype
       getattr(self, f"reco_unmatched_T_{ptype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, self.pt_hat)
       getattr(self, f"reco_unmatched_{ctype}_{ptype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, self.pt_hat)
    ...
    for t_pair in truth_pairs:
      ptype = t_pair.ptype
      getattr(self, f"gen_T_{ptype}").Fill(t_pair.weight, t_pair.r, t_pair.pt, self.pt_hat)

      for d_pair in det_pairs:
        if d_pair.is_equal(t_pair): # no need to check ctype, but ptype is checked
          getattr(self, f"reco_T_{ptype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, self.pt_hat)
          getattr(self, f"response_T_{ptype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, t_pair.weight, t_pair.r, t_pair.pt, self.pt_hat)
          break
      else: # miss if break is not encountered
        getattr(self, f"response_T_{ptype}").Miss(t_pair.weight, t_pair.r, t_pair.pt, self.pt_hat)
    ...
    for t_pair in truth_pairs:
      ctype = t_pair.ctype
      ptype = t_pair.ptype
      getattr(self, f"gen_{ctype}_{ptype}").Fill(t_pair.weight, t_pair.r, t_pair.pt, self.pt_hat)

      for d_pair in det_pairs:
        if d_pair.is_equal(t_pair) and d_pair.is_same_ctype(t_pair):
          getattr(self, f"reco_{ctype}_{ptype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, self.pt_hat)
          getattr(self, f"response_{ctype}_{ptype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, t_pair.weight, t_pair.r, t_pair.pt, self.pt_hat)
          break
      else: # miss if break is not encountered
        getattr(self, f"response_{ctype}_{ptype}").Miss(t_pair.weight, t_pair.r, t_pair.pt, self.pt_hat)
