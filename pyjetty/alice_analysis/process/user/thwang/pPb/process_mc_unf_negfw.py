...
class ProcessMC_ENC(process_mc_base_pPb.ProcessMCBase):
  ...
  def fill_matched_jet_histograms(self, det_jet, truth_jet, rho_bge_det, rho_bge_truth):
    ...
    for d_pair in det_pairs:
       ...
       ctype = d_pair.ctype
       getattr(self,  "reco_unmatched_T").Fill(d_pair.weight, d_pair.r, d_pair.pt, ptype_f * self.pt_hat)
       getattr(self, f"reco_unmatched_{ctype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, ptype_f * self.pt_hat)
    ...
    for t_pair in truth_pairs:
      ...
      getattr(self, "gen_T").Fill(t_pair.weight, t_pair.r, t_pair.pt, ptype_f * self.pt_hat)

      for d_pair in det_pairs:
        if d_pair.is_equal(t_pair): # no need to check ctype, but ptype is checked
          getattr(self, "reco_T").Fill(d_pair.weight, d_pair.r, d_pair.pt, ptype_f * self.pt_hat)
          getattr(self, "response_T").Fill(d_pair.weight, d_pair.r, d_pair.pt, t_pair.weight, t_pair.r, t_pair.pt, ptype_f * self.pt_hat)
          break
      else: # miss if break is not encountered
        getattr(self, "response_T").Miss(t_pair.weight, t_pair.r, t_pair.pt, ptype_f * self.pt_hat)
    ...
    for t_pair in truth_pairs:
      ctype = t_pair.ctype
      ptype_f = self.get_pair_type_factor(t_pair.ptype)
      getattr(self, f"gen_{ctype}").Fill(t_pair.weight, t_pair.r, t_pair.pt, ptype_f * self.pt_hat)

      for d_pair in det_pairs:
        if d_pair.is_equal(t_pair) and d_pair.is_same_ctype(t_pair):
          getattr(self, f"reco_{ctype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, ptype_f * self.pt_hat)
          getattr(self, f"response_{ctype}").Fill(d_pair.weight, d_pair.r, d_pair.pt, t_pair.weight, t_pair.r, t_pair.pt, ptype_f * self.pt_hat)
          break
      else: # miss if break is not encountered
        getattr(self, f"response_{ctype}").Miss(t_pair.weight, t_pair.r, t_pair.pt, ptype_f * self.pt_hat)
