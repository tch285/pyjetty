...
class ProcessMC_ENCTree:
    ...
    def initialize_trees(self):
        src2br = {'pairs_det':         'pairs_det',
                  'pairs_gen':         'pairs_gen', # gen pairs with jet_in_acceptance = true
                  'pairs_det_matched': 'pairs_det', # det pairs with match_pT > 0
                  'pairs_gen_matched': 'pairs_gen', # gen pairs with match_pT > 0
                  'parts_det':         'parts_det',
                  'parts_det_reco':    'parts_det', # parts with match_pT > 0
                  'parts_gen':         'parts_gen',
                  'parts_gen_reco':    'parts_gen', # parts with match_pT > 0
                  'jets_det':          'jets_det',
                  'jets_gen':          'jets_gen', # gen jets with abs(eta) < 0.5
                  'jets_det_matched':  'jets_det', # det jets with match_pT > 0
                  'jets_gen_matched':  'jets_gen', # gen jetse with match_pT > 0
                  'jets_gen_matchable': 'jets_gen',
        }
        ...
        if 'pairs_det' in self.sources:
            self.trees['pairs_det'] = dfs[src2br['pairs_det']]
        if 'pairs_gen' in self.sources:
            # self.trees['pairs_gen'] = dfs[src2br['pairs_gen']].Filter("abs(jet_in_acceptance) < 0.5")
            self.trees['pairs_gen'] = dfs[src2br['pairs_gen']].Filter("jet_in_acceptance")
        if 'pairs_det_matched' in self.sources:
            self.trees['pairs_det_matched'] = dfs[src2br['pairs_det_matched']].Filter("match_pT > 0")
        if 'pairs_gen_matched' in self.sources:
            self.trees['pairs_gen_matched'] = dfs[src2br['pairs_gen_matched']].Filter("match_pT > 0")
        
        if 'parts_det' in self.sources:
            self.trees['parts_det'] = dfs[src2br['parts_det']]
        if 'parts_det_reco' in self.sources:
            self.trees['parts_det_reco'] = dfs[src2br['parts_det_reco']].Filter("match_pT > 0")
        if 'parts_gen' in self.sources:
            self.trees['parts_gen'] = dfs[src2br['parts_gen']]
        if 'parts_gen_reco' in self.sources:
            self.trees['parts_gen_reco'] = dfs[src2br['parts_gen_reco']].Filter("match_pT > 0")
        
        if 'jets_det' in self.sources:
            self.trees['jets_det'] = dfs[src2br['jets_det']]
        if 'jets_gen' in self.sources:
            self.trees['jets_gen'] = dfs[src2br['jets_gen']].Filter(f"abs(eta) < {0.9 - self.jetR}")
        
        if 'jets_det_matched' in self.sources:
            self.trees['jets_det_matched'] = dfs[src2br['jets_det_matched']].Filter("match_pT > 0")
        if 'jets_gen_matched' in self.sources:
            self.trees['jets_gen_matched'] = dfs[src2br['jets_gen_matched']].Filter("match_pT > 0")
        if 'jets_gen_matchable' in self.sources:
            self.trees['jets_gen_matchable'] = dfs[src2br['jets_gen_matchable']]
...
if __name__ == "__main__":
    ...
    analysis = ProcessMC_ENCTree(
        input_file=args.input_file,
        config_file=args.config_file,
        output_dir=args.output_dir,
        output_filename=args.output_filename,
    )
    analysis.process_mc_trees()
