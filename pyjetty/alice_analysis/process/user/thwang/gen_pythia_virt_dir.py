...
class PythiaGenENC(process_base.ProcessBase):
    ...
    def prepare(self):
        ...
        pTRL_bins, _, _, _, _ = gutils.logbins(self.pTRL_min, self.pTRL_max, self.pTRL_nbins)
            ...
            for jetR in self.jetRs:
                ...
                for ipoint in range(2, self.npoint + 1):
                    ...
                    for cEEC_type in gutils.cEEC_types:
                        ...
                        self.hists['ENC'][jet_level][jetR][ipoint][f'{cEEC_type}_virt'] = ROOT.TH2D(f'h_ENC{ipoint}_{cEEC_type}_virt_jetpT_{jet_level}_R{R_label}_1', f'ENC{ipoint}{cEEC_type}virt;jet p_{{T}} (GeV);virt', self.pT_nbins, pt_bins, self.pTRL_nbins, pTRL_bins)
                    ...
                    if ipoint == 2:
                        ...
                        self.hists['ENC'][jet_level][jetR][ipoint]['PM_virt'] = ROOT.TH2D(f'h_ENC{ipoint}_PM_virt_jetpT_{jet_level}_R{R_label}_1', f'ENC{ipoint}PMvirt;jet p_{{T}} (GeV);virt', self.pT_nbins, pt_bins, self.pTRL_nbins, pTRL_bins)
    ...
    def _fill_hists(self, jet_level, jet, jetR):
        ...
        for ipoint in range(2, self.npoint+1):
            ...
            for index in range(cb1.correlator(ipoint).rs().size()):
                ...
                self.hists['ENC'][jet_level][jetR][ipoint]['T_virt'].Fill(jet.perp(), cb1.correlator(ipoint).rs()[index] * jet.perp(), cb1.correlator(ipoint).weights()[index])
        ...
        if jet_level == "ch":
            for ipoint in range(2, self.npoint+1):
                ...
                for indices, RL, weight in zip(cb1.correlator(ipoint).indices(), cb1.correlator(ipoint).rs(), cb1.correlator(ipoint).weights()):
                    ...
                    if np.all(charges > 0):
                        self.hists['ENC'][jet_level][jetR][ipoint]['P_virt'].Fill(jet.perp(), RL * jet.perp(), weight)
                        self.hists['ENC'][jet_level][jetR][ipoint]['P'].Fill(jet.perp(), RL, weight)
                    elif np.all(charges < 0):
                        self.hists['ENC'][jet_level][jetR][ipoint]['M_virt'].Fill(jet.perp(), RL * jet.perp(), weight)
                        self.hists['ENC'][jet_level][jetR][ipoint]['M'].Fill(jet.perp(), RL, weight)
                    else:
                        if ipoint == 2:
                            self.hists['ENC'][jet_level][jetR][ipoint]['PM_virt'].Fill(jet.perp(), RL * jet.perp(), weight)
                            self.hists['ENC'][jet_level][jetR][ipoint]['PM'].Fill(jet.perp(), RL, weight)
                    self.hists['ENC'][jet_level][jetR][ipoint]['Q_virt'].Fill(jet.perp(), RL * jet.perp(), np.prod(charges) * weight)
