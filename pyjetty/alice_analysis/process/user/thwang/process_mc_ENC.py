        ...
        for pair_type_label in self.pair_type_labels:
            if 'ENC' in observable:
              for ipoint in range(2, 3):
                ...
                # Matched det-level pairs, truth jet selection, truth jet pT in weight
                name = 'h_matched_{}{}{}_DetPairTruSelTruPTWeight_JetPt_R{}_{}'.format(observable, ipoint, pair_type_label, jetR, obs_label)
                pt_bins = linbins(0,200,200)
                RL_bins = logbins(1E-4,1,50)
                h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                h.GetXaxis().SetTitle('p_{T,ch jet}')
