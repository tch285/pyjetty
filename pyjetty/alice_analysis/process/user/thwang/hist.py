...
results_path = "/global/cfs/cdirs/alice/mhwang/cEEC/results"
...
if __name__ == '__main__':
    ...
    # hist1 = Histogram1D(data = [2], weights = [0.6], nbins = 5, bin_min = 1e-4, bin_max = 4, binning = 'log', title = "a:b:c")
    # hist1.calc_error()
    # print(len(hist1))
    ...
    # hists = np.load(results_path+"/hists.npz")
    # qq = hists['qq']
    # fig1, axs1 = plt.subplots()
    # axs1.errorbar(hists['centers'], qq, None, None, 'ro', label = "qq")
    # axs1.errorbar(hists['centers'], hists['pp'] + hists['mm'] - hists['pm'], None, None, 'ko', label = "pp+mm-pm")
    # axs1.errorbar(hists['centers'], hists['tr'], None, None, 'go', label = "tr")
    # axs1.legend()
    # axs1.set_xlabel("R_L")
    # axs1.set_ylabel("weighted cts")
    # axs1.set_title("compare")
    # axs1.set_xscale('log')
    # fig1.savefig(results_path+"/qq.png")
    # print(qq)

    hist, _, zzz = np.histogram2d([1, 2], [3, 4], [[0.5, 1.5, 2.5], [2, 3.5, 4.5, 5.5, 6.5]], weights = [0.5, 15])
    print(hist)
    print(hist[0])