...
class ProcessMCBase(process_base.ProcessBase):
    ...
    def process_mc(self):
        ...
        io_det = process_io_polars.ProcessIO(input_file=self.input_file, tree_dir=tree_dir,
                                      track_tree_name='tree_Particle', use_ev_id_ext=False,
                                      is_jetscape=self.jetscape, event_plane_range=self.event_plane_range,
                                      is_ENC=self.ENC_fastsim, is_det_level=True)
        ...
        if self.jetscape:
            io_det_holes = process_io_polars.ProcessIO(input_file=self.input_file, tree_dir=tree_dir,
                                                track_tree_name='tree_Particle', use_ev_id_ext=False,
        ...
        # a SeriesGroupBy object of fastjet particles per event
        io_truth = process_io_polars.ProcessIO(input_file=self.input_file, tree_dir=tree_dir,
                                        track_tree_name='tree_Particle_gen', use_ev_id_ext=False,
                                        is_jetscape=self.jetscape, event_plane_range=self.event_plane_range,
                                        is_ENC=self.ENC_fastsim, is_det_level=False)
        ...
        # If jetscape, store also the negative status particles (holes)
        if self.jetscape:
            io_truth_holes = process_io_polars.ProcessIO(input_file=self.input_file, tree_dir=tree_dir,
                                                    track_tree_name='tree_Particle_gen', use_ev_id_ext=False,
                                                    is_jetscape=self.jetscape, holes=True,
                                                    event_plane_range=self.event_plane_range)
        ...
        if self.jetscape:
            ...
            # self.df_fjparticles.loc[isnull, 'fj_particles_det'] = pandas.Series([fjext.vectorize_pt_eta_phi_m([], [], [], [], 0)] * isnull.sum()).values
    ...
    def analyze_event(self, fj_particles_det, fj_particles_truth, fj_particles_det_holes=None, fj_particles_truth_holes=None, particles_mcid_det=None, particles_pid_truth=None):
        ...
        if not isinstance(fj_particles_truth, fj.vectorPJ) or not isinstance(fj_particles_det, fj.vectorPJ):
            no_tracks = [
                "has no truth tracks" if not isinstance(fj_particles_truth, fj.vectorPJ) else "",
                "has no det tracks" if not isinstance(fj_particles_det, fj.vectorPJ) else ""
            ]
            msg = ", ".join(m for m in no_tracks if m)
            logger.warning(f"Event {self.event_number} {msg}, event skipped.")
            return
        ...
        if self.ENC_fastsim:
            ...
            for i_truth in range( len(fj_particles_truth) ):
                ...
                for i_det in range( len(fj_particles_det) ):
                    ...
                    if abs(info_det.mcid) == mcid_truth:
                        nmatches += 1
                        if nmatches >= 2:
                            logger.warning(
