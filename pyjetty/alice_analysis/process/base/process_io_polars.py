...
class ProcessIO(common_base.CommonBase):
  ...
  def __init__(self, input_file='', tree_dir='PWGHF_TreeCreator',
    ...
    self.schema = {
      ...
      # 'ev_id': pl.UInt32, # or 64, not sure
      # 'ev_id_ext': pl.UInt32, # or 64, not sure

      'z_vtx_reco': pl.Float32,
      'is_ev_rej': pl.UInt16, # can go up to 4000
      'centrality': pl.UInt8,
      # 'event_plane_angle': 

      'ParticlePt': pl.Float32,
      'ParticleEta': pl.Float32,
      'ParticlePhi': pl.Float32,
      # 'status': pl.UInt8, # not sure the range of allowed values
      'ParticleMCid': pl.Int16,
      'ParticleMCIndex': pl.Int16,
      'ParticlePID': pl.Int16,          # 32-bit integer
      'ParticleCharge': pl.Int8,          # 32-bit integer

      
      'int_col': pl.Int32,          # 32-bit integer
      'long_col': pl.Int64,         # 64-bit integer
      'small_int': pl.Int8,         # 8-bit integer
      'uint_col': pl.UInt32,        # unsigned 32-bit integer
      
      # Other types
      'bool_col': pl.Boolean,       # boolean
      'str_col': pl.Utf8,          # string
      'date_col': pl.Date,         # date
    }
    #print(self)
  ...
  def load_data(self, m=0.1396, reject_tracks_fraction=0., offset_indices=False,
    ...
    logger.info('Converting ROOT trees to Polars DataFrames...')
    ...
    if self.reject_tracks_fraction > 1e-3:
      n_remove = int(reject_tracks_fraction * self.track_df.height)
      logger.warning(f'Removing {n_remove} of {self.track_df.height} tracks from {self.track_tree_name}.')
      np.random.seed()
      indices_remove = np.random.choice(self.track_df.height, n_remove, replace=False)
      self.track_df = self.track_df.filter(~pl.Series(range(len(self.track_df))).is_in(indices_remove))
  ...
  def extract_TTree(self, tree_name, columns):
    with uproot.open(self.input_file)[tree_name] as tree:
      data_dict = tree.arrays(columns, library="np")
    return pl.DataFrame(data_dict)
  
  def check_for_duplicates(self, df, cols, name):
    dupl = df.filter(pl.struct(cols).is_duplicated())
    if dupl.height > 0:
      logger.critical(f"There appear to be {dupl.height} duplicate events in the {name} DF:\n{dupl}")
      sys.exit(1)
    else:
      logger.info(f"No duplicates found in {name} DF.")
  #---------------------------------------------------------------
  ...
  def load_dataframe(self):
    ...
    if not self.skip_event_tree:
      ...
      self.event_df_orig = self.extract_TTree(event_tree_name, self.event_columns)
      logger.debug(f"Events prior to event selection:\n{self.event_df_orig}")
    
      self.check_for_duplicates(self.event_df_orig, self.unique_identifier, "event")
      
      # Apply event selection
      event_criteria = pl.col('is_ev_rej') == 0
      if not self.is_pp: # add centrality condition for AA
        event_criteria &= pl.col('centrality') > self.min_centrality & pl.col('centrality') < self.max_centrality
      if self.event_plane_range: # add event plane condition if specified
        event_criteria &= pl.col("event_plane_angle") > self.event_plane_range[0] & pl.col("event_plane_angle") < self.event_plane_range[1]
      event_df = self.event_df_orig.filter(event_criteria)
      print(event_df)
      logger.info(f"Event selection rejected {self.event_df_orig.height - event_df.height}/{self.event_df_orig.height} events; {event_df.height} events remaining.")

    # Load track tree into dataframe
    track_tree_name = self.tree_dir + self.track_tree_name
    track_df_orig = self.extract_TTree(track_tree_name, self.track_columns)
    # Apply hole selection, in case of jetscape
    if self.is_jetscape:
      if self.holes:
        track_df_orig = track_df_orig.filter(pl.col('status') == -1)
      else:
        track_df_orig = track_df_orig.filter(pl.col('status') == 0)
    
    self.check_for_duplicates(track_df_orig, self.track_columns, "track")
    print(track_df_orig)
    ...
    if self.skip_event_tree:
      ...
      self.track_df = track_df_orig.join(event_df, on=self.unique_identifier)
    print(self.track_df)
    self.check_for_duplicates(self.track_df, self.track_columns, "merged")
  ...
  def group_fjparticles(self, m, offset_indices=False, group_by_evid=True, random_mass=False, min_pt=0.):
    ...
    if group_by_evid:
      ...
      track_df_grouped = self.track_df.group_by(*self.unique_identifier)
      ...
      if self.is_ENC:
        ...
        df_fjparticles = track_df_grouped.map_groups(
          lambda df: self.get_fjparticles(df, m, offset_indices, random_mass, min_pt)
        )
        # df_fjparticles = track_df_grouped.apply(
        # self.get_fjparticles, m=m, offset_indices=offset_indices, random_mass=random_mass, min_pt=min_pt)
      
      logger.info(f'Combined:\n{df_fjparticles}')
      logger.info(df_fjparticles)
      input('p')
  ...
  def get_fjparticles(self, df_tracks, m, offset_indices=False, random_mass=False, min_pt=0.):
    ...
    df_tracks_accepted = df_tracks.filter(pl.col("ParticlePt") > min_pt)

    # print('debug2',df_tracks_accepted)

    m_array = np.full((df_tracks_accepted.height), m)
    ...
    fj_particles = fjext.vectorize_pt_eta_phi_m(
      df_tracks_accepted['ParticlePt'].to_numpy(), df_tracks_accepted['ParticleEta'].to_numpy(),
      df_tracks_accepted['ParticlePhi'].to_numpy(), m_array, user_index_offset)
    if self.is_ENC:
      pass
      # if self.is_det_level:
      ...
      for i, (charge, mcid) in enumerate(df_tracks_accepted.select(['ParticleCharge', 'ParticleMCid']).iter_rows()):
